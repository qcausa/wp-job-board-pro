...
if( ! class_exists( 'CMB2_Field_Ajax_Search' ) ) {
	...
	class CMB2_Field_Ajax_Search {
		...
		public function __construct() {
			...
			// Ajax endpoints.
			add_action( 'wjbp_ajax_cmb_ajax_search_get_results', array( $this, 'get_results' ) );
		...
		public function render( $field, $value, $object_id, $object_type, $field_type ) {
			BugFu::log('render init');
			BugFu::log($object_id);

			$this->setup_admin_scripts();

			if ( get_post_type($object_id) == 'employer' ) {
				$prefix = WP_JOB_BOARD_PRO_EMPLOYER_PREFIX;
			} elseif ( get_post_type($object_id) == 'candidate' ) {
				$prefix = WP_JOB_BOARD_PRO_CANDIDATE_PREFIX;
			}

			$value = get_post_meta( $object_id, $prefix.'user_id', true );
			BugFu::log($value);
		...
		public function sanitize( $override_value, $value, $object_id, $field_args ) {
			...
			if( !empty($field_args['render_row_cb'][0]->data_to_save[$field_args['id']]) ) {
		...
		public function get_results() {
			...
			if ( ! wp_verify_nonce( $nonce, 'cmb_ajax_search_get_results' ) ) {
				...
				die( json_encode( array(
                    'error' => __( 'Error : Unauthorized action', 'wp-job-board-pro' )
                ) ) );
                ...
                die( json_encode( array(
                    'error' => __( 'Error : Wrong request parameters', 'wp-job-board-pro' )
                ) ) );
            } else {
		...
		public function object_text( $field_id, $object_id, $object_type ) {
			...
			if( $object_type == 'post' ) {
				...
				if ( WP_Job_Board_Pro_User::is_employer($object_id) ) {
					$employer_id = WP_Job_Board_Pro_User::get_employer_by_user_id($object_id);
		...
		public function object_text_html( $field_id, $object_id, $object_type ) {
			...
			if( $object_type == 'post' ) {
				...
				if ( $post_type == 'employer' || $post_type == 'candidate' ) {
					...
					$avatar = get_the_post_thumbnail( $object_id, 'thumbnail' );
