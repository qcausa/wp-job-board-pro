...
class WP_Job_Board_Pro_Candidate {
	...
	public static function insert_applicant($user_id, $job, $cv_file_ids = 0) {
		...
		if ($applicant_id) {
			...
			// MODIFIED: Separate filter for job creator email
			$send_email_to_creator = apply_filters('wp_job_board_pro_send_application_email_to_creator', true, $job_id, $candidate_id, $user_id);
	
			if ($send_email_to_creator === true) {
				...
				$result = WP_Job_Board_Pro_Email::wp_mail($author_email, $email_subject, $email_content, $headers, $files_path);
				// end send email to job creator
			}
	
			// thanks email to applicant
